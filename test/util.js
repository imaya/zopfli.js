...
function arrayEquals(expected, actuals) {
  ...
  if (expected.length !== actuals.length) {
    console.log("wrong length:" + expected.length + ", " + actuals.length);
    return false;
  }

  for (i = 0, il = expected.length; i < il; ++i) {
    if (expected[i] !== actuals[i]) {
      console.log("Array diff: index=" + i + ", " + expected[i] + ', ' + actuals[i]);
      return false;
    }
  }
...
function makeRandomData(data, seed) {
  var mt = new MersenneTwister(seed = seed || +new Date());
  var i, il;
